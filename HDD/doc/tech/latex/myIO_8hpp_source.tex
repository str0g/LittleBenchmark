...
00235     \}
00236     \textcolor{comment}{//List}
00237     \textcolor{keyword}{inline} \textcolor{keywordtype}{void} lsFiles(\textcolor{keyword}{const} boost::filesystem::path &bfsp\_dir,\(\backslash\)
00238                         std::list<boost::filesystem::path> *list\_dir)\{
00239         \textcolor{keywordflow}{if}( boost::filesystem::exists( bfsp\_dir ) )\{
00240             boost::filesystem::directory\_iterator end ;
00241             \textcolor{keywordflow}{for}( boost::filesystem::directory\_iterator iter(bfsp\_dir);\(\backslash\)
00242                     iter != end; ++iter )
00243                 \textcolor{keywordflow}{if}( boost::filesystem::is\_regular\_file( *iter ))\{
00244                     list\_dir->push\_back( iter->path() );
00245                 \}
00246         \}\textcolor{keywordflow}{else}\{
00247             std::cerr<<bfsp\_dir<<\textcolor{stringliteral}{" does not exist"}<<std::endl;
00248         \}
00249     \}
00250     \textcolor{comment}{//COPY}
00251     \textcolor{keyword}{inline} \textcolor{keywordtype}{bool} CopyFileByChar(\textcolor{keyword}{const} boost::filesystem::path &pathFrom,\(\backslash\)
00252                                \textcolor{keyword}{const} boost::filesystem::path &pathTo)\{
00253         \textcolor{keywordflow}{if} ( boost::filesystem::is\_directory(pathFrom) )\{std::cerr<<pathFrom<<\textcolor{stringliteral}{" i
      s a directory file requered"}<<std::endl; \textcolor{keywordflow}{return} \textcolor{keyword}{false}; \}
00254         \textcolor{keywordflow}{if} ( boost::filesystem::is\_directory(pathTo) )\{ std::cerr<<pathFrom<<\textcolor{stringliteral}{" is
       a directory file requered"}<<std::endl; \textcolor{keywordflow}{return} \textcolor{keyword}{false}; \}
00255 
00256         boost::filesystem::ifstream fileFrom (pathFrom);
00257         boost::filesystem::ofstream fileTo (pathTo,std::ios::ate);
00258         std::string strError;
00259         \textcolor{keywordflow}{if} (fileFrom.is\_open() and fileTo.is\_open())\{
00260             \textcolor{keywordtype}{char} c;
00261             \textcolor{keywordflow}{while} (fileFrom.get(c))\{
00262                 fileTo.put(c);
00263                 \textcolor{keywordflow}{if} ( fileTo.fail() ) \{ std::cerr << \textcolor{stringliteral}{"Write error:"}<<pathFrom<<std
      ::endl; \textcolor{keywordflow}{return} \textcolor{keyword}{false}; \}
00264             \}
00265             \textcolor{keywordflow}{if} ( !fileFrom.eof() ) \{ std::cerr << \textcolor{stringliteral}{"Read error:"}<<pathTo<<std::end
      l; \textcolor{keywordflow}{return} \textcolor{keyword}{false}; \}
00266             \textcolor{keywordflow}{return} \textcolor{keyword}{true};
00267         \}
00268         \textcolor{keywordflow}{if} (fileFrom.is\_open())\{ fileFrom.close(); \} \textcolor{keywordflow}{else} \{ strError.append(\textcolor{stringliteral}{" "}+p
      athFrom.file\_string()); \}
00269         \textcolor{keywordflow}{if} (fileTo.is\_open())\{ fileTo.close(); \} \textcolor{keywordflow}{else} \{ strError.append(\textcolor{stringliteral}{" "}+pathT
      o.file\_string()); \}
00270         std::cerr<<\textcolor{stringliteral}{"Failed to copy files by char"}<<strError<<std::endl;
00271         \textcolor{keywordflow}{return} \textcolor{keyword}{false};
00272 
00273     \}
00274     \textcolor{keyword}{inline} \textcolor{keywordtype}{bool} ReadByChunk()\{
00275         \textcolor{keywordflow}{return} \textcolor{keyword}{false};
00276     \}
00277 \}
00278 
00279 \textcolor{preprocessor}{#endif // MYIO\_HPP\_INCLUDED}
